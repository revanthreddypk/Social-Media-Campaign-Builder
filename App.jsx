...
export default function App() {
  ...
  const showKeyPrompt = aiEnabled === false;
    ...
    <div className="cc">
      <div className="wrap">
        ...
        <div className="lede">
          ...
          <p>Fill in the brief below and build a full plan across your chosen channels. With an API key connected, the AI writes the strategy from your exact inputs and unlocks ad copy, creative hooks and a landing-page audit — using any provider you like.</p>
        </div>

        {/* ---------- KEY PROMPT (only when no key connected) ---------- */}
        {showKeyPrompt &&
          <div className="keybanner">
            <span className="kb-ic"><Spark /></span>
            <div className="kb-txt">
              <b>Add an API key for the best output.</b> Right now you'll get a full plan built from benchmarks — free. Connect any AI provider's key (Anthropic, OpenAI or Google) and the AI writes the strategy tailored to your business and unlocks ad copy, creative hooks and a landing-page audit.
            </div>
          </div>}
        ...
        <div className="card console">
          ...
          <div className="form-grid">
            <label className="f spanfull"><span>Business — name &amp; what it does <b className="req">*</b></span>
            ...
            <label className="f"><span>Offer (optional)</span>
              <input type="text" value={f.offer} onChange={(e) => set("offer", e.target.value)} placeholder="e.g. Free consult + 10% off" /></label>

            <label className="f spanfull"><span>Product / brief</span>
        ...
        <div className="out">
          {!plan &&
            <div className="empty card">
              ...
              <p>Fill in the brief above and press <b>Build my plan</b>.</p>
            </div>}
          ...
          {plan &&
            <div className="plan">
              ...
              <Block n="2" title={`Budget split · AED ${fmt(plan.split.totalBudget)} ${plan.inputs.period === "total" ? "total" : "/mo"}`}>
              ...
              <Block n="3" title="Campaign structure per platform">
              ...
              <Block n="4" title="Target audience per platform" ai={!!aiPlan?.audiences?.length}>
                {mAudiences.map((a) => (
                  <div key={a.p} className="stack"><div className="stack-h">{a.short}</div><p className="ptxt">{a.text}</p></div>))}
              </Block>

              <Block n="5" title="Audience & messaging angles" ai={!!aiPlan?.angles?.length}>
                {mAngles.map((a, i) => (
                  <div className="angle" key={i}><span className="ai">{i + 1}</span><p><b>{a.t}</b> — <span>{a.d}</span></p></div>))}
              </Block>

              <Block n="6" title="Creative types per platform" ai={!!aiPlan?.creativeDirection?.length}>
              ...
              <Block n="12" title="Optimization & scaling rules" ai={!!aiPlan?.optimization?.length}>
